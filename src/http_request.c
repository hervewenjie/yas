...
static int yas_http_process_ignore(yas_http_request_t *r, yas_http_out_t *out, char *data, int len);
static int yas_http_process_connection(yas_http_request_t *r, yas_http_out_t *out, char *data, int len);
static int yas_http_process_if_modified_since(yas_http_request_t *r, yas_http_out_t *out, char *data, int len);

yas_http_header_handle_t yas_http_headers_in[] = {
    {"Host", yas_http_process_ignore},
    {"Connection", yas_http_process_connection},
    {"If-Modified-Since", yas_http_process_if_modified_since},
    {"", yas_http_process_ignore}
};
...
void yas_http_handle_header(yas_http_request_t *r, yas_http_out_t *o) {
    list_head *pos;
    yas_http_header_t *hd;
    yas_http_header_handle_t *header_in;
    int len;

    list_for_each(pos, &(r->list)) {
        hd = list_entry(pos, yas_http_header_t, list);
        /* handle */

        for (header_in = yas_http_headers_in;
...
static int yas_http_process_if_modified_since(yas_http_request_t *r, yas_http_out_t *out, char *data, int len) {
    ...
    if (fabs(time_diff) < 1e-6) {
        ...
        /* Not modified */
        out->modified = 0;
        out->status = YAS_HTTP_NOT_MODIFIED;
    }
...
const char *get_shortmsg_from_status_code(int status_code) {
    ...
    */
    if (status_code == YAS_HTTP_OK) {
        return "OK";
    }

    if (status_code == YAS_HTTP_NOT_MODIFIED) {
        return "Not Modified";
    }

    if (status_code == YAS_HTTP_NOT_FOUND) {
        return "Not Found";
    }
