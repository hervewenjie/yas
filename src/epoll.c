...
void yas_epoll_add(int epfd, int fd, struct epoll_event *event) {
    int rc = epoll_ctl(epfd, EPOLL_CTL_ADD, fd, event);
    check(rc == 0, "zv_epoll_add: epoll_ctl");
    return;
}

void yas_epoll_mod(int epfd, int fd, struct epoll_event *event) {
    int rc = epoll_ctl(epfd, EPOLL_CTL_MOD, fd, event);
    check(rc == 0, "zv_epoll_mod: epoll_ctl");
    return;
}

void yas_epoll_del(int epfd, int fd, struct epoll_event *event) {
